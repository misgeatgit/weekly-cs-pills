  ...
  def train max_epoch
    ...
    until epoch < max_epoch do
      for d in 0..@X.size()-1
        ...
        layers[0].input = @X[d]
        outputs = layers[0].compute_output()
        for i in  1..layers.size()-2
          layers[i].input = outputs
        ...
        @ouput_layer.update_weights()
        i=@hidden_layers.size()-1
        while i >= 0 do
          sigma = @hidden_layers[i].compute_sigma(sigma,@utput_layer.weights)
          @hidden_layers[i].update_weights()
        end
      end
    end
  end
